...
export default class StaticComponent extends BaseComponent {
    ...
    constructor(tag: string | null, props: StaticProps, body: StaticBody | null = null) {
        super()
        this.props = props
        this.tagName = tag
        this.debugName = tag
    ...
    on(eventName: string, fn: (e: Event) => void): this {
        ...
        this.events[eventName] = (e: Event) => {
            if (this.node instanceof Node && (e.target === this.node || this.node.contains(e.target as Node))) {
        ...
        if (this.tagName === "input" || this.tagName === "textarea") {
            if (value instanceof Ref) {
                ...
                this.on("input", (e: Event) => {
                    const inputTarget = e.target as HTMLInputElement | HTMLTextAreaElement
